...
auto is_default_op(Operations op) -> bool { return (op != Operations::Power) && (op != Operations::Remainder); }

auto get_op_name(Operations op)
{
...
struct Calculator
{
        using Ops = std::vector<Instruction>;
        ...
        Calculator()
        {
                ops.reserve(static_cast<int>(Operations::Count));        // reserve space for the operations we have

                // add our default operations
                ops.emplace_back(Operations::Addition, [](float lhs, float rhs) { return lhs + rhs; });
                ops.emplace_back(Operations::Subtraction, [](float lhs, float rhs) { return lhs - rhs; });
                ops.emplace_back(Operations::Multiplication, [](float lhs, float rhs) { return lhs * rhs; });
                ops.emplace_back(Operations::Division, [](float lhs, float rhs) { return lhs / rhs; });
        }

        auto compute(Operations op, float lhs, float rhs) -> std::optional<float>
        {
                const auto pinstr = std::find_if(ops.begin(), ops.end(), [op](const auto& i) { return i.op == op; });
                if (pinstr != ops.end()) { return {pinstr->fn(lhs, rhs)}; }
        ...
        auto add_new_op(Operations op)
        {
                ...
                if (!is_default_op(op))
                {
                        ...
                        if (std::none_of(ops.begin(), ops.end(), [&](const auto& i) { return i.op == op; }))
                        {
                                // add the requested operation
                                if (op == Operations::Power)
                                {
                                        ops.emplace_back(op, [](float lhs, float rhs) { return std::pow(lhs, rhs); });
                                }
                                else if (op == Operations::Remainder)
                                {
                                        ops.emplace_back(op, [](float lhs, float rhs) { return std::fmod(lhs, rhs); });
                                }
        ...
        auto print_ops()
        {
                ...
                for (auto& i : ops)
                {
                        auto name = get_op_name(i.op).data();
